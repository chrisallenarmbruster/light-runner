...
var mode = "show", //default mode.  Command line option -m:string will override.
  cycles = 5, //default base cycles per scene.  Command line option -c:number will override.
  delay = 50, //in ms, default base delay between state changes (on/off duration - speed of routine).  Command line option -d:number will override.
...
const server = http.createServer((request, response) => {
  ...
  response.end(
    `lightRunner.js is running\nprogram mode: ${mode}\nlight state: ${state}\nscheduled wake time (mins): ${startTime}\nscheduled sleep time (mins): ${stopTime}`
  );
});

server.listen(3000);

//process command line arguments given at startup and override defaults
    ...
    switch (process.argv[i][1]) {
      ...
      case "h": //help - show usage help
        console.log(
          ...
          "example: node lightRunner.js -m:show -c:5 -d:50\n"
        );
        allLightsOff();
        process.exit();
      default:
...
switch (mode) {
  ...
  case "popcorn":
    ...
    break;
  default:
    console.log('Mode not valid. Run "node lightRunner -h" for help.\n');
    allLightsOff();
    process.exit();
}
